...
<head>
    ...
    function openStaffOrderManage(orderId, status = '') {
        window.location.href = '<?php echo $base_path; ?>//printflow/staff/customizations.php?order_id=' + orderId + '&status=' + encodeURIComponent(status) + '&job_type=ORDER';
    }
    ...
    function openOrderModal(orderId) {
        ...
        fetch('<?php echo $base_path; ?>//printflow/staff/get_order_data.php?id=' + orderId, {
    ...
    function approveDesign(orderId, csrfToken) {
        ...
        fetch('<?php echo $base_path; ?>//printflow/staff/approve_design_process.php', {
            method: 'POST', body: fd,
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
    ...
    function verifyPayment(orderId, action) {
        ...
        fetch('<?php echo $base_path; ?>//printflow/staff/api_verify_payment.php', {
            method: 'POST', body: fd,
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(function(r) { 
    ...
    function markAsCompleted(orderId, csrfToken) {
        ...
        fetch('<?php echo $base_path; ?>//printflow/staff/orders.php', {
            method: 'POST', body: fd,
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(function(r) { 
        ...
        .then(function(res) {
            if (res.success) {
                ...
                setTimeout(function() { 
                    closeOrderModal();
                    window.location.href = '<?php echo $base_path; ?>//printflow/staff/orders.php?status=Completed';
                }, 1500);
            } else {
    ...
    function renderOrderModal(d) {
        ...
        if (d.payment_proof) {
            ...
            if (paymentUrl.includes('<?php echo $base_path; ?>//printflow/printflow/')) {
                paymentUrl = paymentUrl.replace('<?php echo $base_path; ?>//printflow/printflow/', '/printflow/');
            }
