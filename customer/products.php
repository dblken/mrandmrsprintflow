...
$sql = "SELECT p.*, 
    ...
    WHERE p.status = 'Activated'
      AND COALESCE(LOWER(TRIM(p.product_type)), 'fixed') = 'fixed'";
$params = [];
$types = '';
...
$count_sql = "SELECT COUNT(*) as total FROM products WHERE status = 'Activated' AND COALESCE(LOWER(TRIM(product_type)), 'fixed') = 'fixed'";
$count_params = [];
$count_types = '';
