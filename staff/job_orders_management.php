                            ...
                            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 12px;">
                                <template x-for="file in (currentJo.files || [])" :key="file.id">
                                    <div style="width:100%; margin-bottom:12px; text-align: center;">
                                        <a :href="' + (window.PFConfig?.basePath || '') + '/' + file.file_path.replace(/^\/+/, '')" target="_blank" style="display: block; border-radius: 12px; overflow: hidden; border: 2px solid #f1f5f9; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1);">
                                            <img :src="'<?php echo $base_path; ?>/' + file.file_path.replace(/^\/+/, '')" alt="Design" style="width: 100%; max-height: 250px; object-fit: cover; display: block;">
                                        </a>
                                        <a :href="' + (window.PFConfig?.basePath || '') + '/' + file.file_path.replace(/^\/+/, '')" target="_blank" style="display: inline-block; font-size: 12px; color: #06A1A1; margin-top: 8px; font-weight: 700; text-decoration: none; background: #e6f7f5; padding: 4px 10px; border-radius: 6px;">↗ View Full</a>
                                    </div>
                                </template>
                            </div>
                        </div>
