...
var __pfReportsChartRootIds = ['salesChart','ch-forecast','ch-products','ch-donut','ch-category-products','ch-category-custom','ch-category-dashboard','ch-custom','ch-status'];
...
window.printflowInitReportsCharts = function () {
    ...
    (function initCategoryDashboardChart(){
        try {
            ...
            const sections = [
                { key: 'products', mountId: 'ch-category-products', chartId: 'pf-ch-category-products', sourceMode: 'store' },
                { key: 'custom', mountId: 'ch-category-custom', chartId: 'pf-ch-category-custom', sourceMode: 'job' },
                { key: 'combined', mountId: 'ch-category-dashboard', chartId: 'pf-ch-category-dashboard', sourceMode: 'combined' }
            ];
            ...
            const buildSourceLine = function(section, row) {
                ...
                if (section.sourceMode === 'store') {
                    return 'Products: <strong style="color:#f8fafc;">\u20b1' + storeRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 }) + '</strong> • ' + storeQty.toLocaleString() + ' units';
                }
                if (section.sourceMode === 'job') {
                    return 'Job Orders: <strong style="color:#f8fafc;">\u20b1' + jobRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 }) + '</strong> • ' + jobQty.toLocaleString() + ' units';
                }
                return 'Store: <strong style="color:#f8fafc;">\u20b1' + storeRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 }) + '</strong> • ' + storeQty.toLocaleString() + ' units'
                    + '<br><span>Custom: <strong style="color:#f8fafc;">\u20b1' + jobRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 }) + '</strong> • ' + jobQty.toLocaleString() + ' units</span>';
            };

            const renderCategorySection = function(section) {
                const dataset = catDash[section.key] || {};
                const rows = Array.isArray(dataset.rows) ? dataset.rows.slice(0, 8) : [];
                ...
                pfPushApexChart(mount, {
                    chart:{ ...PF_OPT, id:section.chartId, redrawOnParentResize:true, type:'bar', height:chartHeight },
                    ...
                    tooltip:{
                        ...
                        custom:function(ctx) {
                            ...
                            h += '<div>' + buildSourceLine(section, row) + '</div>';
                            h += '</div>';
                            h += '</div>';
                            return h;
                        }
            ...
            sections.forEach(renderCategorySection);
