...
if (get_user_type() === 'Customer' && !is_customer_id_verified()) {
    ...
    $verify_url = $base_path . '/customer/profile.php#section-id';
    $services_url = $base_path . '/customer/services.php';
    ...
    echo '<p style="color:#64748b;font-size:.95rem;line-height:1.6;margin-bottom:2rem;">' . htmlspecialchars($msg, ENT_QUOTES, 'UTF-8') . '</p>';
    if ($id_status !== 'Pending') {
        echo '<a href="' . htmlspecialchars($verify_url, ENT_QUOTES, 'UTF-8') . '" style="display:inline-block;background:#0a2530;color:#fff;font-weight:700;padding:.85rem 2rem;border-radius:.75rem;text-decoration:none;margin-bottom:1rem;">Verify My ID</a><br>';
    }
    echo '<a href="' . htmlspecialchars($services_url, ENT_QUOTES, 'UTF-8') . '" style="color:#64748b;font-size:.875rem;text-decoration:none;">&larr; Back to Services</a>';
    echo '</div></div>';
