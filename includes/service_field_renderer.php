...
function render_service_field($field_key, $config, $branches = [], $existing_data = []) {
    ...
    switch ($config['type']) {
        ...
        case 'textarea':
            $html .= '<textarea name="' . htmlspecialchars($field_key) . '" rows="6" class="shopee-opt-btn notes-textarea" placeholder="Any special instructions..." maxlength="500" ' . $required_attr . ' style="max-width: 600px; height: 160px; resize: none; align-items: flex-start; justify-content: flex-start; text-align: left; padding: 0.75rem;">' . htmlspecialchars($saved_value) . '</textarea>';
            break;
