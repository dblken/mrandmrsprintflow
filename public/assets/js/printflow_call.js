/**
 * PrintFlow WebRTC Call System v4.5 (Global & Stable)
 * =================================================
 */
...
class PFAudio {
    ...
    play(basePath) {
        ...
        // Use a more reliable ringtone source
...
class PFCallManager {
    ...
    init(config) {
        if (window.__PF_CALL_INIT__ && this.userId === config.userId) {
            this._ensureUI();
            return;
        }
    ...
    _connectSocket() {
        ...
        // Use standard WSS if on production, raw port 3000 if local
        ...
        this.socket.on('connect', () => {
            ...
            console.log('[PFCall] Socket Connected:', this.socket.id);
        });
    ...
    async startCall(targetId, targetType, targetName, targetAvatar, type = 'voice') {
        ...
        try {
            ...
            if (this.state !== PF_STATE.CALLING) return; // Cancelled
            ...
            console.error('[PFCall] Media Error:', err);
    ...
    _handleIncomingCall(data) {
        ...
        // Show Messenger-style Toast if not on chat page or minimized
        ...
        // Auto-reject after 35s if no action
        this._noAnswerTimeout = setTimeout(() => {
    ...
    _hideToast() {
        this.$('pf-call-toast')?.classList.remove('active');
    }

    _flashEnded(msg) {
        this._showOverlay(PF_STATE.ENDED, null, null, msg);
