...
switch ($action) {
    ...
    case 'callback':
        ...
        $res = login_customer_by_google($email, $firstName, $lastName);
        if (!empty($res['success'])) {
            $dest = $res['redirect'] ?? (defined('BASE_PATH') ? (BASE_PATH . '/customer/services.php') : '/customer/services.php');
            header('Location: ' . $dest);
            exit;
        }
        redirect(BASE_PATH . '/public/login.php?error=' . urlencode($res['message'] ?? 'Google sign-in failed. Please try again.'));
        break;
